...
async def _fetch_nominatim_hotels(canonical_city: str, *, limit: int = 20) -> list[HotelOffer]:
    ...
    for item in raw_items:
        ...
        base_price_usd = _rng.uniform(20, 100)
...
def _parse_hotel(prop: dict, check_in: date, check_out: date, city: str) -> HotelOffer:
    ...
    if price_usd_per_night <= 0:
        ...
        price_usd_per_night = _hotel_rng(city, _nm).uniform(45, 180)
...
def _parse_hotels_com_hotel(prop: dict, city: str) -> HotelOffer | None:
    ...
    try:
        ...
        price_usd = 0.0
    if price_usd <= 0:
        price_usd = _hotel_rng(city, name).uniform(45, 180)
    price_pkr = round(price_usd * settings.USD_TO_PKR_RATE, 2)
...
async def _fetch_hotels_com_rooms(
    ...
    for room in rooms_raw[:8]:
        ...
        try:
            ...
            price_usd = 0.0
        if price_usd <= 0:
            price_usd = random.uniform(45, 180)
        price_pkr = round(price_usd * settings.USD_TO_PKR_RATE, 2)
...
async def search_hotels(
    ...
    # A live source (Google Places, TripAdvisor, ...) can return the real name
    # of a hotel we've separately researched a specific rate for, but priced
    # with only a generic tier guess. When the names clearly match, prefer the
    # researched figure over the generic one.
    _apply_curated_price_overrides(hotels)
...
_HOTEL_DATA: list[dict] = [
    # Karachi — hotel names verified live via Google Places API; prices from
    # Kayak/HotelsCombined/Momondo current listings (bookme.pk carries no
    # branded-chain inventory for these cities — see hotel pricing session notes)
    {"id": "KHI-001", "name": "Pearl Continental Karachi", "city": "Karachi",
     "stars": 5, "address": "Club Rd, Karachi", "price_usd": 105,
    ...
    {"id": "KHI-002", "name": "Movenpick Hotel Karachi", "city": "Karachi",
     "stars": 5, "address": "Club Rd, Karachi", "price_usd": 115,
    ...
    {"id": "KHI-003", "name": "Karachi Marriott Hotel", "city": "Karachi",
     "stars": 5, "address": "9 Abdullah Haroon Road, Civil Lines, Karachi", "price_usd": 150,
     "lat": 24.8580, "lon": 67.0090,
     "amenities": ["Pool", "Gym", "Restaurant", "Airport Shuttle", "Business Center"],
     "images": ["https://images.unsplash.com/photo-1445019980597-93fa8acb246c?w=800"]},
    {"id": "KHI-004", "name": "Avari Towers Karachi", "city": "Karachi",
     "stars": 5, "address": "Fatima Jinnah Rd, Karachi", "price_usd": 105,
    ...
    {"id": "LHE-001", "name": "Pearl Continental Lahore", "city": "Lahore",
     "stars": 5, "address": "Shahrah-e-Quaid-e-Azam, Lahore", "price_usd": 105,
    ...
    {"id": "LHE-002", "name": "Avari Hotel Lahore", "city": "Lahore",
     "stars": 5, "address": "87 Shahrah-e-Quaid-e-Azam, Lahore", "price_usd": 95,
     "lat": 31.5497, "lon": 74.3436,
     "amenities": ["Pool", "Gym", "Restaurant", "Free WiFi", "Parking"],
     "images": ["https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=800"]},
    {"id": "LHE-003", "name": "Four Points by Sheraton Lahore", "city": "Lahore",
     "stars": 4, "address": "25 Egerton Road, Garhi Shahu, Lahore", "price_usd": 90,
     "lat": 31.5520, "lon": 74.3320,
     "amenities": ["Restaurant", "Free WiFi", "Airport Shuttle", "Parking", "Business Center"],
    ...
    {"id": "LHE-004", "name": "Faletti's Hotel Lahore", "city": "Lahore",
     "stars": 4, "address": "Egerton Rd, Lahore", "price_usd": 75,
    ...
    {"id": "ISB-001", "name": "Serena Hotel Islamabad", "city": "Islamabad",
     "stars": 5, "address": "Khayaban-e-Suharwardy, Islamabad", "price_usd": 165,
    ...
    {"id": "ISB-002", "name": "Marriott Hotel Islamabad", "city": "Islamabad",
     "stars": 5, "address": "Agha Khan Rd, Islamabad", "price_usd": 160,
     "lat": 33.7121, "lon": 73.0765,
     "amenities": ["Pool", "Spa", "Gym", "Restaurant", "Business Center", "Valet"],
     "images": ["https://images.unsplash.com/photo-1611892440504-42a792e24d32?w=800"]},
    {"id": "ISB-003", "name": "Envoy Continental Hotel", "city": "Islamabad",
     "stars": 4, "address": "111-East AKM Fazl-ul-Haq Road, Blue Area, Islamabad", "price_usd": 65,
     "lat": 33.7150, "lon": 73.0850,
     "amenities": ["Restaurant", "Free WiFi", "Parking", "Room Service", "Gym"],
     "images": ["https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=800"]},

    # Multan
    {"id": "MUL-001", "name": "Hotel One Multan", "city": "Multan",
     "stars": 4, "address": "Abdali Road, Multan", "price_usd": 70,
    ...
    {"id": "MUL-002", "name": "Ramada Multan", "city": "Multan",
     "stars": 4, "address": "Nishtar Road, Multan", "price_usd": 75,
    ...
    {"id": "PEW-001", "name": "Pearl Continental Peshawar", "city": "Peshawar",
     "stars": 5, "address": "Khyber Rd, Peshawar", "price_usd": 160,
    ...
    {"id": "QTA-001", "name": "Serena Hotel Quetta", "city": "Quetta",
     "stars": 5, "address": "Shahrah-e-Zarghoon, Quetta", "price_usd": 140,
    ...
    {"id": "FSD-001", "name": "Hotel One Faisalabad", "city": "Faisalabad",
     "stars": 4, "address": "Susan Road, Faisalabad", "price_usd": 65,
    ...
    # Skardu (prices in PKR equivalent)
    {"id": "SKD-001", "name": "Serena Hotel Skardu", "city": "Skardu",
     "stars": 5, "address": "Sadpara Road, Skardu", "price_usd": 95,
     "lat": 35.2971, "lon": 75.6338,
     "amenities": ["Mountain View", "Restaurant", "Jeep Rental", "Hiking Guide", "Free WiFi"],
     "images": ["https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800"]},
    # Real PKR rate (not converted from USD): standard rooms ~PKR 29,500-32,000/night per current listings.
    {"id": "SKD-006", "name": "Shangrila Resort Skardu", "city": "Skardu",
     "stars": 5, "address": "Kachura Road, Skardu", "price_usd": 108,
     "lat": 35.4436, "lon": 75.4636,
     "amenities": ["Lakefront", "Mountain View", "Restaurant", "Garden", "Free WiFi"],
     "images": ["https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800"]},
    {"id": "SKD-002", "name": "Mashabrum Hotel Skardu", "city": "Skardu",
     "stars": 3, "address": "Airport Road, Skardu", "price_usd": 37,
    ...
    {"id": "SKD-003", "name": "K2 Motel Skardu", "city": "Skardu",
     "stars": 2, "address": "Yadgar Chowk, Skardu", "price_usd": 13,
    ...
    {"id": "GIL-001", "name": "Serena Hotel Gilgit", "city": "Gilgit",
     "stars": 4, "address": "Jutial, Gilgit", "price_usd": 110,
    ...
    {"id": "GIL-002", "name": "PTDC Motel Gilgit", "city": "Gilgit",
     "stars": 3, "address": "KKH Road, Gilgit", "price_usd": 50,
    ...
    {"id": "HNZ-001", "name": "Eagle's Nest Hotel Hunza", "city": "Hunza",
     "stars": 4, "address": "Duikar, Upper Hunza", "price_usd": 110,
    ...
    {"id": "HNZ-002", "name": "Serena Karimabad Hunza", "city": "Hunza",
     "stars": 4, "address": "Karimabad, Hunza", "price_usd": 110,
    ...
    {"id": "SWT-001", "name": "Serena Hotel Swat", "city": "Swat",
     "stars": 5, "address": "Saidu Sharif Road, Mingora", "price_usd": 140,
    ...
    {"id": "SWT-002", "name": "PC Swat", "city": "Swat",
     "stars": 5, "address": "Malam Jabba, Swat", "price_usd": 140,
    ...
    {"id": "ABT-001", "name": "Pearl Continental Abbottabad", "city": "Abbottabad",
     "stars": 5, "address": "Shimla Hill, Abbottabad", "price_usd": 140,
    ...
    {"id": "ABT-002", "name": "Hotel One Abbottabad", "city": "Abbottabad",
     "stars": 3, "address": "Mansehra Road, Abbottabad", "price_usd": 55,
    ...
    {"id": "MRE-001", "name": "PC Murree", "city": "Murree",
     "stars": 5, "address": "The Mall, Murree", "price_usd": 140,
    ...
    {"id": "MRE-002", "name": "Hotel One Murree", "city": "Murree",
     "stars": 3, "address": "Kashmir Point Road, Murree", "price_usd": 55,
    ...
    {"id": "NTG-001", "name": "Greens Hotel Nathiagali", "city": "Nathiagali",
     "stars": 3, "address": "Main Road, Nathiagali", "price_usd": 50,
    ...
    {"id": "NTG-002", "name": "Pines Hotel Nathiagali", "city": "Nathiagali",
     "stars": 3, "address": "Governor's House Road, Nathiagali", "price_usd": 40,
    ...
    {"id": "MZD-001", "name": "PC Muzaffarabad", "city": "Muzaffarabad",
     "stars": 5, "address": "AJK Road, Muzaffarabad", "price_usd": 140,
    ...
    {"id": "MZD-002", "name": "Hilton Muzaffarabad", "city": "Muzaffarabad",
     "stars": 4, "address": "Neelum Road, Muzaffarabad", "price_usd": 75,
    ...
    {"id": "BWP-001", "name": "Hotel One Bahawalpur", "city": "Bahawalpur",
     "stars": 3, "address": "Baghdad ul Jadeed, Bahawalpur", "price_usd": 50,
     "lat": 29.3956, "lon": 71.6836,
     "amenities": ["Restaurant", "Free WiFi", "Parking", "Room Service"],
     "images": ["https://images.unsplash.com/photo-1568084680786-a84f91d1153c?w=800"]},
    {"id": "BWP-002", "name": "Cholistan Desert Resort", "city": "Bahawalpur",
     "stars": 3, "address": "Derawar Fort Road, Bahawalpur", "price_usd": 55,
    ...
    {"id": "NRN-001", "name": "PTDC Motel Naran", "city": "Naran",
     "stars": 3, "address": "Main Bazar, Naran", "price_usd": 55,
    ...
    {"id": "HNZ-005", "name": "Baltit Fort View Hotel", "city": "Hunza",
     "stars": 3, "address": "Near Baltit Fort, Karimabad", "price_usd": 55,
    ...
    {"id": "GIL-003", "name": "Park Hotel Gilgit", "city": "Gilgit",
     "stars": 3, "address": "Bank Road, Gilgit", "price_usd": 40,
    ...
    {"id": "GIL-004", "name": "Jasmine Guest House Gilgit", "city": "Gilgit",
     "stars": 3, "address": "Jutial, Gilgit", "price_usd": 40,
    ...
    {"id": "SWT-003", "name": "White Palace Hotel Swat", "city": "Swat",
     "stars": 4, "address": "Mingora, Swat", "price_usd": 90,
    ...
    {"id": "ABT-003", "name": "Sarban Hotel Abbottabad", "city": "Abbottabad",
     "stars": 3, "address": "Mall Road, Abbottabad", "price_usd": 50,
    ...
    {"id": "MUR-003", "name": "Shangrila Resort Murree", "city": "Murree",
     "stars": 4, "address": "Bhurban Road, Murree", "price_usd": 90,
    ...
    {"id": "MUR-004", "name": "Cecil Hotel Murree", "city": "Murree",
     "stars": 3, "address": "The Mall, Murree", "price_usd": 55,
    ...
    {"id": "NTG-003", "name": "PTDC Motel Nathiagali", "city": "Nathiagali",
     "stars": 2, "address": "PTDC, Nathiagali", "price_usd": 25,
    ...
    {"id": "MZD-003", "name": "Neelum Valley Hotel", "city": "Muzaffarabad",
     "stars": 3, "address": "Neelum Chowk, Muzaffarabad", "price_usd": 50,
    ...
    {"id": "HYD-001", "name": "Hotel Faran Hyderabad", "city": "Hyderabad",
     "stars": 3, "address": "Saddar, Hyderabad", "price_usd": 50,
     "lat": 25.3960, "lon": 68.3578,
     "amenities": ["Restaurant", "Free WiFi", "AC", "Parking"],
     "images": ["https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800"]},
    {"id": "HYD-002", "name": "Hotel City Gate Hyderabad", "city": "Hyderabad",
     "stars": 3, "address": "Autobahn Road, Hyderabad", "price_usd": 40,
    ...
    {"id": "SKT-001", "name": "Hotel One Sialkot", "city": "Sialkot",
     "stars": 4, "address": "Cantt, Sialkot", "price_usd": 70,
     "lat": 32.4945, "lon": 74.5229,
     "amenities": ["Restaurant", "Free WiFi", "Pool", "Gym", "Parking"],
     "images": ["https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?w=800"]},
    {"id": "SKT-002", "name": "Tariq Hotel Sialkot", "city": "Sialkot",
     "stars": 3, "address": "Shahabpura Road, Sialkot", "price_usd": 45,
     "lat": 32.4930, "lon": 74.5210,
     "amenities": ["Restaurant", "Free WiFi", "AC", "Parking"],
     "images": ["https://images.unsplash.com/photo-1445019980597-93fa8acb246c?w=800"]},
]

_HOTEL_NAME_STOPWORDS = {"hotel", "hotels", "the", "by", "and", "resort", "inn", "guest", "house"}


def _hotel_name_tokens(name: str, city: str) -> set[str]:
    """
    Significant words in a hotel name, for matching the same hotel across
    sources whose naming differs slightly (accents, word order, city suffix,
    "Hotel"/"by"/etc.). Strips accents (Mövenpick -> movenpick) and drops the
    city name itself, since a live source may omit or include it inconsistently.
    """
    text = unicodedata.normalize("NFKD", name).encode("ascii", "ignore").decode("ascii").lower()
    text = text.replace("-", " ").replace("'", " ").replace(",", " ")
    stop = _HOTEL_NAME_STOPWORDS | {city.strip().lower()}
    return {t for t in text.split() if t and t not in stop and len(t) >= 2}


def _apply_curated_price_overrides(hotels: list["HotelOffer"]) -> None:
    """
    A live source (Google Places, TripAdvisor, ...) can return a hotel's real
    name priced only by a generic tier guess (e.g. Google's price_level band).
    `_HOTEL_DATA` carries specific researched nightly rates for a handful of
    well-known chain hotels — when a live result's name clearly matches one of
    those, its researched rate is the better number to show, so it replaces
    the generic guess (each room's price is rescaled by the same ratio, so a
    source's own Standard/Deluxe spread is preserved).
    """
    curated_by_city: dict[str, list[dict]] = {}
    for entry in _HOTEL_DATA:
        curated_by_city.setdefault(entry["city"], []).append(entry)

    for hotel in hotels:
        candidates = curated_by_city.get(hotel.city)
        if not candidates or not hotel.rooms:
            continue
        live_tokens = _hotel_name_tokens(hotel.name, hotel.city)
        if not live_tokens:
            continue
        for entry in candidates:
            curated_tokens = _hotel_name_tokens(entry["name"], entry["city"])
            if curated_tokens and curated_tokens <= live_tokens:
                price_pkr = round(entry["price_usd"] * settings.USD_TO_PKR_RATE, 2)
                base_price = hotel.rooms[0].price_per_night_pkr or price_pkr
                for room in hotel.rooms:
                    ratio = (room.price_per_night_pkr or base_price) / base_price
                    room.price_per_night_pkr = round(price_pkr * ratio, 2)
                break
    ...
    try:
        ...
        for idx, el in enumerate(elements[:20]):   # cap at 20 OSM results
            ...
            _lo, _hi = {
                "hotel": (45.0, 180.0),
                "hostel": (10.0, 30.0),
                "guest_house": (18.0, 55.0),
            }.get(tourism_type, (45.0, 45.0))
